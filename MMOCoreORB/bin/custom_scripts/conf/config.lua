--*******************************************************************************
--Copyright (C) 2015 - 2017 <SWG Awakening>

--This File is part of the SWG Awakening project.

--This file can not be copied and/or distributed via ANY medium without 
--express written permission from the owner of SWG Awakening.
--*******************************************************************************

--************************************************
--Settings 0 = false, 1 = true
--************************************************
-- 0 = false, 1 = true
MakeLogin = 1
MakeZone = 1
MakePing = 1
MakeStatus = 1
MakeWeb = 0

--************************************************
--Zone/Character Configuration
--************************************************
ProgressMonitors = "true"
ZoneProcessingThreads = 10
ZoneAllowedConnections = 30000
ZoneGalaxyID = 1
ZoneOnlineCharactersPerAccount = 2 --How many characters are allowed online from a single account.
PurgeDeletedCharacters = 10 --How often in minutes to purge deleted characters
ORB = ""

--************************************************
--Game Server Database Configuration
--************************************************
DBHost = "localhost"
DBPort = 10389
DBName = "swglive"
DBUser = "root"
DBPass = "DATABASE PASS"
DBSecret = "swgemus3cr37!" -- Change this! This value should be unique and of reasonable length.

MantisHost = "localhost"
MantisPort = 10389
MantisName = "swglive"
MantisUser = "root"
MantisPass = "DATABASE PASS"
MantisPrfx = "mantis_" -- The prefix for your mantis tables.

--************************************************
--Login Server Configuration
--************************************************
LoginPort = 44453
LoginProcessingThreads = 1
LoginAllowedConnections = 3000
--AutoReg = 0 --Uncomment if live server.
--AutoReg = 1 --Uncomment if test server.
LoginRequiredVersion = "20050408-18:00"

--************************************************
--Status Server Configuration
--************************************************
StatusPort = 44455
StatusAllowedConnections = 500
StatusInterval = 30 -- interval to check if zone is locked up (in seconds)

--************************************************
--Web Server Configuration
--************************************************
WebPorts = 44460 -- Can be multiple ports 44460,44461
WebAccessLog = "../log/webaccess.log"
WebErrorLog = "../log/weberror.log"
WebSessionTimeout = 600 -- Length that inactive web sessions expire

--************************************************
--Ping Configuration
--************************************************
PingPort = 44462
PingAllowedConnections = 3000

--************************************************
--Tre Files Path
--Tre files that are commented out are custom and not part of Core3.
--Core3 only needs the default tre files to run.
--************************************************
TrePath = "/home/swgemu/tre"

TreFiles = {
	"default_patch.tre",
	"fhemu4.tre",
	"fhemu3.tre",
	"fhemu2.tre",
	"patch_sku1_14_00.tre",
	"patch_14_00.tre",
	"fhemu1.tre",
	"patch_sku1_13_00.tre",
	"patch_13_00.tre",
	"patch_sku1_12_00.tre",
	"patch_12_00.tre",
	"patch_11_03.tre",
	"data_sku1_07.tre",
	"patch_11_02.tre",
	"data_sku1_06.tre",
	"patch_11_01.tre",
	"patch_11_00.tre",
	"data_sku1_05.tre",
	"data_sku1_04.tre",
	"data_sku1_03.tre",
	"data_sku1_02.tre",
	"data_sku1_01.tre",
	"data_sku1_00.tre",
	"patch_10.tre",
	"patch_09.tre",
	"patch_08.tre",
	"patch_07.tre",
	"patch_06.tre",
	"patch_05.tre",
	"patch_04.tre",
	"patch_03.tre",
	"patch_02.tre",
	"patch_01.tre",
	"patch_00.tre",
	"data_other_00.tre",
	"data_static_mesh_01.tre",
	"data_static_mesh_00.tre",
	"data_texture_07.tre",
	"data_texture_06.tre",
	"data_texture_05.tre",
	"data_texture_04.tre",
	"data_texture_03.tre",
	"data_texture_02.tre",
	"data_texture_01.tre",
	"data_texture_00.tre",
	"data_skeletal_mesh_01.tre",
	"data_skeletal_mesh_00.tre",
	"data_animation_00.tre",
	"data_sample_04.tre",
	"data_sample_03.tre",
	"data_sample_02.tre",
	"data_sample_01.tre",
	"data_sample_00.tre",
	"data_music_00.tre",
	"bottom.tre"
}

--************************************************
--Planet Configuration
--************************************************
--The following zones are enabled, and will be loaded with server start.
--To save on RAM and CPU usage, you should only enable the zones you need.
--In order to disable a zone, all you have to do is comment it out.
ZonesEnabled = {
	--"09",
	--"10",
	--"11",
	--"character_farm",
	--"cinco_city_test_m5",
	  "corellia",
	--"creature_test",
	  "dantooine",
	  "dathomir",
	  "dungeon1",
	  "endor",
	--"endor_asommers",
	--"floratest",
	--"godclient_test",
	  "lok",
	  "naboo",
	--"otoh_gunga",
	--"rivertest",
	  "rori",
	--"runtimerules",
	--"simple",
	--"space_09",
	--"space_corellia",
	--"space_corellia_2",
	--"space_dantooine",
	--"space_dathomir",
	--"space_endor",
	--"space_env",
	--"space_halos",
	--"space_heavy1",
	--"space_light1",
	--"space_lok",
	--"space_naboo",
	--"space_naboo_2",
	--"space_tatooine",
	--"space_tatooine_2",
	--"space_yavin4",
	--"taanab",
	  "talus",
	  "tatooine",
	--"test_wearables",
	  "tutorial",
	--"umbra",
	--"watertabletest",
	  "yavin4"
}
