...
local SpawnMobiles = require("scripts.utils.spawn_mobiles")
...
WiseOldManEncounter = Encounter:new {
	-- Task properties
	taskName = "WiseOldManEncounter",
	-- Encounter properties
	minimumTimeUntilEncounter = 10 * 60 * 1000, -- 10 Minutes
	maximumTimeUntilEncounter = 60 * 60 * 1000, -- 60 Minutes
...
function WiseOldManEncounter:getPlayerFirstName(pCreatureObject)
...
-- @param pCreatureObject pointer to the creature object of the player.
function WiseOldManEncounter:sendGreetingString(pOldMan, pCreatureObject)
...
function WiseOldManEncounter:onEncounterInRange(pCreatureObject, oldManPointerList)
...
function WiseOldManEncounter:handleScheduledDespawn(pCreatureObject)
...
-- @param pCreatureObject pointer to the creature object of the player.
function WiseOldManEncounter:scheduleDespawnOfOldMan(pCreatureObject)
	if (pCreatureObject == nil) then
		return
	end

	Logger:log("Scheduling despawn of old man.", LT_INFO)
	createEvent(OLD_MAN_DESPAWN_TIME, "WiseOldManEncounter", "handleScheduledDespawn", pCreatureObject, "")
end

-- Give the force crystal to the player.
-- @param pCreatureObject pointer to the creature object of the player.
function WiseOldManEncounter:giveForceCrystalToPlayer(pCreatureObject)
	if (pCreatureObject == nil) then
		return
	end
...
function WiseOldManEncounter:hasForceCrystal(pCreatureObject)
...
-- @param pCreatureObject pointer to the creature object of the player.
function WiseOldManEncounter:removeForceCrystalFromPlayer(pCreatureObject)
	if (pCreatureObject == nil) then
		return
	end
...
function WiseOldManEncounter:doesOldManBelongToThePlayer(pConversingPlayer, pConversingOldMan)
	if (pConversingPlayer == nil or pConversingOldMan == nil) then
		return false
	end

	local playerOldMan = SpawnMobiles.getSpawnedMobiles(pConversingPlayer, WiseOldManEncounter.taskName)
...
function WiseOldManEncounter:isEncounterFinished(pCreatureObject)
	if (pCreatureObject == nil) then
		return
	end
...
-- @param pCreatureObject pointer to the creature object of the player.
function WiseOldManEncounter:taskFinish(pCreatureObject)
	if (pCreatureObject == nil) then
		return true
	end
