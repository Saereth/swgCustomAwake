tusken_savage = Creature:new {
	objectName = "@mob/creature_names:tusken_savage",
	socialGroup = "tusken_raider",
	faction = "tusken_raider",
	level = 20,
	chanceHit = 0.5, --SWG Awakening Custom -- Chance Hit
	damageMin = 290, --SWG Awakening Custom -- Min Damage
	damageMax = 600, --SWG Awakening Custom -- Max Damage
	baseXp = 1803,
	baseHAM = 5000,
	baseHAMmax = 7500, --SWG Awakening Custom -- Base Ham Max
	armor = 0,
	--resists = {35,-1,10,30,-1,30,-1,-1,-1},
	resists = {35,30,20,30,25,30,25,20,-1}, --SWG Awakening Custom -- Resists
	meatType = "",
	meatAmount = 0,
	hideType = "",
	hideAmount = 0,
	boneType = "",
	boneAmount = 0,
	milk = 0,
	tamingChance = 0,
	ferocity = 0,
	pvpBitmask = AGGRESSIVE + ATTACKABLE + ENEMY,
	creatureBitmask = PACK + KILLER + STALKER,
	optionsBitmask = AIENABLED,
	diet = HERBIVORE,

	templates = {"object/mobile/tusken_raider.iff"},
	lootGroups = {
		{
			groups = {
				{group = "junk", chance = 1500000},
				{group = "tusken_common", chance = 3000000},
				{group = "wearables_common", chance = 2000000},
				{group = "bone_armor", chance = 750000},
				{group = "chitin_armor", chance = 750000},
				{group = "armor_attachments", chance = 500000},
				{group = "clothing_attachments", chance = 500000},
				{group = "color_crystals", chance = 500000},
				{group = "crystals_poor", chance = 500000}
			}
		}
	},
	weapons = {"tusken_weapons"},
	conversationTemplate = "",
	attacks = merge(marksmanmaster,brawlermaster,carbineermaster)
}

CreatureTemplates:addCreatureTemplate(tusken_savage, "tusken_savage")
