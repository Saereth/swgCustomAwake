...
void LoginPacketHandler::handleDeleteCharacterMessage(LoginClient* client, Message* pack) {
    ...
    if(!dbDelete){
    	try {
    		Reference<ResultSet*> deleteResults1 = ServerDatabase::instance()->executeQuery(deleteStatement);

    		if(deleteResults1 == NULL || deleteResults1.get()->getRowsAffected() == 0) {
    			StringBuffer errMsg;
    			errMsg << "ERROR: Unable to delete character from the character table. " << endl;
    			errMsg << "QUERY: " << deleteStatement.toString();
    			dbDelete = 1;
    		}

    	//SWG Awakening Custom -- Jedi System, Delete Jedi Unlock Variables From Database On Character Deletion
		if (useCustom && dbDelete == 1) {
    			Reference<ResultSet*> deleteResults2 = ServerDatabase::instance()->executeQuery(deleteJediUnlockStatement);
    			if (deleteResults2 == NULL || deleteResults2.get()->getRowsAffected() == 0) {
    				StringBuffer errMsg;
