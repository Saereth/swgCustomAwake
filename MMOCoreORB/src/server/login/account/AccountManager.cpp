...
void AccountManager::loginAccount(LoginClient* client, Message* packet) {
	...
	//SWG Awakening Custom -- Revision Error, Defined In Server Settings Config
...
Account* AccountManager::validateAccountCredentials(LoginClient* client, const String& username, const String& password) {
	...
	if(account == NULL) {
		...
		if (isAutoRegistrationEnabled() && client != NULL) {
			...
			//SWG Awakening Custom -- Auto Registration Error, Defined In Server Settings Config
			if (client != NULL)
	...
	if(!account->isActive()) {
		...
		//SWG Awakening Custom -- Read Account Disabled Error, Defined In Server Settings Config
		if (client != NULL)
	...
	if (passwordStored != passwordHashed) {
		...
		//SWG Awakening Custom -- Wrong Password Error, Defined In Server Settings Config
		if (client != NULL)
	...
	if(account->isBanned()) {
		...
		//SWG Awakening Custom -- Banned Error, Defined In Server Settings Config
		StringBuffer reason;
