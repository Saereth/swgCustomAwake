...
bool CombatManager::startCombat(CreatureObject* attacker, TangibleObject* defender, bool lockDefender) {
	...
	if (creo != NULL && creo->isIncapacitated() && !creo->isFeigningDeath())
		return false;
...
int CombatManager::getAttackerAccuracyModifier(TangibleObject* attacker, CreatureObject* defender, WeaponObject* weapon) {
	...
	//SWG Awakening Custom -- Force Sensitive melee_accuracy Skill Mod
...
int CombatManager::getDefenderDefenseModifier(CreatureObject* defender, WeaponObject* weapon, TangibleObject* attacker) {
	...
	//SWG Awakening Custom -- Force Sensitive melee_defence Skill Mod
	if (weapon->getAttackType() == WeaponObject::MELEEATTACK) {
...
float CombatManager::applyDamageModifiers(CreatureObject* attacker, WeaponObject* weapon, float damage, const CreatureAttackData& data) {
	...
	int feignDeathMultipler = attacker->getSkillMod("private_feign_death_damage_multiplier");
	int stunMultiplier = attacker->getSkillMod("private_stunned_damage_multiplier");

	int damageMultiplier = feignDeathMultipler + stunMultiplier;

	if (damageMultiplier != 0)
		damage *= damageMultiplier;

	//SWG Awakening Custom --  Temporary State Debuff Stack Fix
	int feignDeathDivisor = attacker->getSkillMod("private_feign_death_damage_divisor");
	int stunDivisor = attacker->getSkillMod("private_stunned_damage_divisor");
	int intimidateDivisor = attacker->getSkillMod("private_intimidate_damage_divisor");
	int forceRunDivisor = attacker->getSkillMod("private_force_run_damage_divisor");

	int damageDivisor = feignDeathDivisor + stunDivisor + intimidateDivisor + forceRunDivisor;
...
int CombatManager::getHitChance(TangibleObject* attacker, CreatureObject* targetCreature, WeaponObject* weapon, int damage, int accuracyBonus) {
	...
	if (damage > 0) {
		...
		//SWG Awakening Custom -- Master Ranged Profession Bonus
		int hitChanceIncrease = 0;

		if (creoAttacker != NULL) {
			if (creoAttacker->hasSkill("combat_bountyhunter_master") || creoAttacker->hasSkill("combat_carbine_master") || creoAttacker->hasSkill("combat_commando_master") ||
					creoAttacker->hasSkill("combat_marksman_master") || creoAttacker->hasSkill("combat_pistol_master")  || creoAttacker->hasSkill("combat_rifleman_master")) {
		...
		if (def == "saber_block") {
			if (!attacker->isTurret() && (weapon->getAttackType() == WeaponObject::RANGEDATTACK) && ((System::random(100) + hitChanceIncrease) < targetCreature->getSkillMod(def)))
				return RICOCHET;
			else return HIT;
		}
...
Reference<SortedVector<ManagedReference<TangibleObject*> >* > CombatManager::getAreaTargets(TangibleObject* attacker, WeaponObject* weapon, TangibleObject* defenderObject, const CreatureAttackData& data) {
	...
	try {
		...
		for (int i = 0; i < closeObjects.size(); ++i) {
			...
			if (creo != NULL && !creo->isFeigningDeath() && creo->isIncapacitated()) {
