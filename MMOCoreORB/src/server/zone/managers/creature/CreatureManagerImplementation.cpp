	...
	try {
		...
		if (player != NULL) {
			...
			if (player->isPlayerCreature()) {
				...
				//SWG Awakening Custom -- Knight Trials, Trigger Creature Killed Observer
				...
				if (group != NULL) {
					for (int i = 0; i < group->getGroupSize(); i++) {
						...
						if (groupMember != NULL) {
							...
							if (groupGhost != NULL && groupGhost->getKnightTrialInProgress()) {
								if (groupMember->getWorldPosition().distanceTo(destructedObject->getWorldPosition()) < 40) {
									groupMember->notifyObservers(ObserverEventType::TRIALKILLEDCREATURE, destructedObject);
								}
							}
						}
					}
					} else {
						if (ghost != NULL && ghost->getKnightTrialInProgress())
							player->notifyObservers(ObserverEventType::TRIALKILLEDCREATURE, destructedObject);
					}
