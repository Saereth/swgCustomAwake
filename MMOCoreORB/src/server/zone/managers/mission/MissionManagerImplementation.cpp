...
void MissionManagerImplementation::randomizeGenericBountyMission(CreatureObject* player, MissionObject* mission, const uint32 faction) {
	...
	if (npcTarget) {
		...
		if (target != NULL) {
			...
			//SWG Awakening Custom -- Bounty Missions Do Not Show For Same Guild Or Same Ip Addresses
			if (playerGhost != NULL && creatureGhost != NULL) {
				if (playerGhost->getIpAddress() == creatureGhost->getIpAddress()) {
					return;
				}
				if (player->isInGuild() && creature->isInGuild()) {
					if (player->getGuildID() == creature->getGuildID()) {
						return;
					}
				}
			}
