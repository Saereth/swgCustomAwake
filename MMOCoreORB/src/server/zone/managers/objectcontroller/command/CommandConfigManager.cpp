...
void CommandConfigManager::parseVariableData(String varName, LuaObject &command, QueueCommand* slashCommand) {
	...
	if (varName == "name") // just ignore name, it's only used to grab the object from the table
		...
		else if (varName == "intimDefenceDebuff")
			combatCommand->setIntimDefenceDebuff(Lua::getIntParameter(L));
		else if (varName == "intimDamageDivisor")
			combatCommand->setIntimDamageDivisor(Lua::getIntParameter(L));
...
void CommandConfigManager::registerCommands() {
	...
	//SWG Awakening Custom -- Register Custom Commands
	commandFactory.registerCommand<RemovePvpStatesCommand>(String("removePvpStates").toLowerCase());
	commandFactory.registerCommand<GetKnightShrineCommand>(String("getKnightShrine").toLowerCase());
	commandFactory.registerCommand<ResetKnightTrialsCommand>(String("resetKnightTrials").toLowerCase());
	commandFactory.registerCommand<SetKnightTrialCommand>(String("setKnightTrial").toLowerCase());
}
