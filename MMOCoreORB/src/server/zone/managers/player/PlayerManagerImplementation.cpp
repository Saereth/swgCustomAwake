...
PlayerManagerImplementation::PlayerManagerImplementation(ZoneServer* zoneServer, ZoneProcessServer* impl) :
	...
	//SWG Awakening Custom -- Advanced Logging
	//Admin Trade Log
	adminTradeLog.setLoggingName("Admin Trade");
	StringBuffer adminTradeFileName;
	adminTradeFileName << "log/admin/trade.log";
	adminTradeLog.setFileLogger(adminTradeFileName.toString(), true);
	adminTradeLog.setLogging(true);

	//Player Trade Log
	playerTradeLog.setLoggingName("Player Trade");
	StringBuffer playerTradeFileName;
	playerTradeFileName << "log/player/trade.log";
	playerTradeLog.setFileLogger(playerTradeFileName.toString(), true);
	playerTradeLog.setLogging(true);

	//Auction Chat Log
	auctionChatLog.setLoggingName("Auction Chat");
	StringBuffer auctionChatFileName;
	auctionChatFileName << "log/chat/auctionchat.log";
	auctionChatLog.setFileLogger(auctionChatFileName.toString(), true);
	auctionChatLog.setLogging(true);

	//Planet Chat Log
	planetChatLog.setLoggingName("Planet Chat");
	StringBuffer planetChatFileName;
	planetChatFileName << "log/chat/planetchat.log";
	planetChatLog.setFileLogger(planetChatFileName.toString(), true);
	planetChatLog.setLogging(true);

	//Spatial Chat Log
	spatialChatLog.setLoggingName("Spatial Chat");
	StringBuffer spatialChatFileName;
	spatialChatFileName << "log/chat/spatialchat.log";
	spatialChatLog.setFileLogger(spatialChatFileName.toString(), true);
	spatialChatLog.setLogging(true);

	//General Chat Log
	generalChatLog.setLoggingName("General Chat");
	StringBuffer generalChatFileName;
	generalChatFileName << "log/chat/generalchat.log";
	generalChatLog.setFileLogger(generalChatFileName.toString(), true);
	generalChatLog.setLogging(true);

	//Imperial Chat Log
	imperialChatLog.setLoggingName("Imperial Chat");
	StringBuffer imperialChatFileName;
	imperialChatFileName << "log/chat/imperialchat.log";
	imperialChatLog.setFileLogger(imperialChatFileName.toString(), true);
	imperialChatLog.setLogging(true);

	//Rebel Chat Log
	rebelChatLog.setLoggingName("Rebel Chat");
	StringBuffer rebelChatFileName;
	rebelChatFileName << "log/chat/rebelchat.log";
...
void PlayerManagerImplementation::loadLuaConfig() {
	...
	//SWG Awakening Custom -- Staff Buffs
...
bool PlayerManagerImplementation::kickUser(const String& name, const String& admin, int duration, String& reason) {
	...
	//Ban player for defined duration
	if (duration > 0) {
		String banMessage = banAccount(adminghost, getAccount(ghost->getAccountID()), duration, reason);
...
void PlayerManagerImplementation::sendActivateCloneRequest(CreatureObject* player, int typeofdeath) {
	...
	ManagedReference<SceneObject*> secondClosestCloning = NULL;
	...
	//SWG Awakening Custom -- City Alignment System, Neutral City Cloning Option
	if (ServerSettings::instance()->getCityTefEnabled()) {
		if (cr != NULL && cr->getFactionAlignment() != 0 && player->getFaction() != 0 && cr->getFactionAlignment() != player->getFaction()) {
			for (int j = 0; j < locations.size(); j++) {
				ManagedReference<SceneObject*> location = locations.get(j);
				...
				if (cr != NULL) {
					...
					if (cr->getFactionAlignment() != 0 && player->getFaction() != 0 && cr->getFactionAlignment() != player->getFaction())
						continue;
	...
	//SWG Awakening Custom -- City Alignment System, Neutral City Cloning Option Text
	if (secondClosestCloning != NULL) {
	...
	//SWG Awakening Custom -- City Alignment System, Add Neutral City Cloning Menu Option
	if (secondClosestCloning != NULL)
	...
	//SWG Awakening Custom -- Jedi System, Add Force Shrine Cloning Menu Option
	if (ghost->getJediState() >= 2)
		cloneMenu->addMenuItem("Revive At Force Shrine");
...
void PlayerManagerImplementation::sendPlayerToCloner(CreatureObject* player, uint64 clonerID, int typeofdeath) {
	...
	//SWG Awakening Custom -- TEF System, Set Faction Player To Covert Upon Cloning
	if (ServerSettings::instance()->getTefEnabled()) {
		if (!ghost->getJediState() >= 4 && player->getFaction() != 0 && ghost->getFactionStatus() != FactionStatus::COVERT && cbot->getFaction() == 0)
			ghost->setFactionStatus(FactionStatus::COVERT);
	} else {
		if (!ghost->getJediState() >= 4 && ghost->getFactionStatus() != FactionStatus::ONLEAVE && cbot->getFaction() == 0)
	...
	//SWG Awakening Custom -- Spawn Protection, Grant Spawn Protection Upon Cloning
	if (ServerSettings::instance()->getSpawnProtectionEnabled())
	...
	//SWG Awakening Custom -- Jedi System, Calculate Jedi Experience Loss Upon Cloning
	if (ghost->getJediState() >= 2) {
		int jediSkillPoints = ghost->getSpentJediSkillPoints();
		int xpLoss = (int)(jediSkillPoints * -800);
...
void PlayerManagerImplementation::awardExperience(CreatureObject* player, const String& xpType,
	...
	//SWG Awakening Custom -- Jedi System, Jedi Experience Divisor Defined In Server Settings Config
	float jediXpDivisor = ServerSettings::instance()->getJediXpDivisor();

	if (xpType == "jedi_general" && amount > 0) {
		xp = playerObject->addExperience(xpType, (int) (amount * localMultiplier * globalExpMultiplier / jediXpDivisor));
	} else if (xpType == "jedi_general" && amount < 0) {
...
void PlayerManagerImplementation::handleVerifyTradeMessage(CreatureObject* player) {
	...
	if (obj != NULL && obj->isPlayerCreature()) {
		...
		if (receiverTradeContainer->hasVerifiedTrade()) {
			...
			for (int i = 0; i < tradeContainer->getTradeSize(); ++i) {
				...
				if (item->isTangibleObject()) {
					if (objectController->transferObject(item, receiverInventory, -1, true)) {
						...
						//SWG Awakening Custom -- Advanced Logging, Trigger Logging On All Staff Trades
						if (ghost != NULL && ghost->hasGodMode()) {
							...
							logPlayerAction("adminTradeLog", logEntry.toString());
						}
					}
				} else {
			...
			for (int i = 0; i < receiverTradeContainer->getTradeSize(); ++i) {
				...
				if (item->isTangibleObject()) {
					if (objectController->transferObject(item, playerInventory, -1, true)) {
						...
						//SWG Awakening Custom -- Advanced Logging, Trigger Logging On All Staff Trades
						if (receiverGhost != NULL && receiverGhost->hasGodMode()) {
							StringBuffer logEntry;
			...
			//SWG Awakening Custom -- Advanced Logging, Trigger Logging On All Large Credit Trades
			if (giveMoney >= 5000000) {
				StringBuffer logEntry;
				logEntry << player->getFirstName() << " traded " << giveMoney << " credits to " << receiver->getFirstName();
				logPlayerAction("playerTradeLog", logEntry.toString());
			}

			//SWG Awakening Custom -- Advanced Logging, Trigger Logging On All Staff Credit Trades
			if (ghost != NULL && ghost->hasGodMode() && giveMoney > 0) {
				StringBuffer logEntry;
				logEntry << player->getFirstName() << " traded " << giveMoney << " credits to " << receiver->getFirstName();
				logPlayerAction("adminTradeLog", logEntry.toString());
			}
			...
			//SWG Awakening Custom -- Advanced Logging, Trigger Logging On All Staff Credit Trades
			if (receiverGhost != NULL && receiverGhost->hasGodMode() && giveMoney > 0) {
				StringBuffer logEntry;
				logEntry << receiver->getFirstName() << " traded " << giveMoney << " credits to " << player->getFirstName();
				logPlayerAction("adminTradeLog", logEntry.toString());
			}
	...
	if (currentPermissionLevel != 0) {
		...
		if(skillsToBeRemoved != NULL) {
			for(int i = 0; i < skillsToBeRemoved->size(); i++) {
				...
				skillManager->surrenderSkill(skill, targetPlayer, true, false);
			}
		}
	}
...
String PlayerManagerImplementation::banAccount(PlayerObject* admin, Account* account, uint32 minutes, const String& reason) {
	...
	try {
		StringBuffer query;
		query << "INSERT INTO account_bans values (NULL, " << account->getAccountID() << ", " << admin->getAccountID() << ", now(), " << (uint64)time(0) + (60 * minutes) << ", '" << escapedReason << "');";
...
//SWG Awakening Custom -- Staff Buffs, Enhance Character
...
//SWG Awakening Custom -- Enhancement Terminal, Enhance Character
void PlayerManagerImplementation::terminalEnhanceCharacter(CreatureObject* player) {
	if (player == NULL)
		return;

	bool message = true;
	terminalPerformanceBuff = ServerSettings::instance()->getTerminalPerformanceBuff();
	terminalMedicalBuff = ServerSettings::instance()->getTerminalMedicalBuff();
	terminalPerformanceDuration = ServerSettings::instance()->getTerminalPerformanceDuration();
	terminalMedicalDuration = ServerSettings::instance()->getTerminalMedicalDuration();
...
//SWG Awakening Custom -- Character Builder Terminal, Enhance Character
void PlayerManagerImplementation::builderEnhanceCharacter(CreatureObject* player) {
	if (player == NULL)
		return;

	bool message = true;
	builderPerformanceBuff = ServerSettings::instance()->getBuilderPerformanceBuff();
	builderMedicalBuff = ServerSettings::instance()->getBuilderMedicalBuff();
	builderPerformanceDuration = ServerSettings::instance()->getBuilderPerformanceDuration();
	builderMedicalDuration = ServerSettings::instance()->getBuilderMedicalDuration();
...
//SWG Awakening Custom -- Advanced Logging, Log Player Action Function
void PlayerManagerImplementation::logPlayerAction(const String& logName, const String& message) {
	if (logName == "adminTradeLog") {
		adminTradeLog.info(message);
	} else if (logName == "playerTradeLog") {
		playerTradeLog.info(message);
	} else if (logName == "auctionChatLog") {
		auctionChatLog.info(message);
	} else if (logName == "planetChatLog") {
		planetChatLog.info(message);
	} else if (logName == "spatialChatLog") {
		spatialChatLog.info(message);
	} else if (logName == "generalChatLog") {
		generalChatLog.info(message);
	} else if (logName == "imperialChatLog") {
...
//SWG Awakening Custom -- Jedi System, Send Player To Force Shrine Function
void PlayerManagerImplementation::sendPlayerToShrine(CreatureObject* player) {
	Zone* zone = player->getZone();
	Coordinate* coordinate = new Coordinate();

	if (zone->getZoneName() == "corellia") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-7391, 0, -3938);
		else if (randomShrine == 1)
			coordinate->setPosition(6092, 0, -5578);
		else if (randomShrine == 2)
			coordinate->setPosition(-6907, 0, 4527);
		else if (randomShrine == 3)
			coordinate->setPosition(-2384, 0, 6393);
		else
			coordinate->setPosition(6300, 0, 6687);
	} else 	if (zone->getZoneName() == "tatooine") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-6505, 0, -3667);
		else if (randomShrine == 1)
			coordinate->setPosition(5264, 0, 113);
		else if (randomShrine == 2)
			coordinate->setPosition(5632, 0, 6015);
		else if (randomShrine == 3)
			coordinate->setPosition(5958, 0, -5685);
		else
			coordinate->setPosition(-3622, 0, 5280);
	} else 	if (zone->getZoneName() == "dantooine") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-6173, 0, -4120);
		else if (randomShrine == 1)
			coordinate->setPosition(2163, 0, 7548);
		else if (randomShrine == 2)
			coordinate->setPosition(2640, 0, -1536);
		else if (randomShrine == 3)
			coordinate->setPosition(-6999, 0, -5269);
		else
			coordinate->setPosition(-1814, 0, -6202);
	} else 	if (zone->getZoneName() == "endor") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-5055, 0, -1703);
		else if (randomShrine == 1)
			coordinate->setPosition(-5627, 0, 4813);
		else if (randomShrine == 2)
			coordinate->setPosition(-3870, 0, -4467);
		else if (randomShrine == 3)
			coordinate->setPosition(670, 0, 5548);
		else
			coordinate->setPosition(5116, 0, 1923);
	} else 	if (zone->getZoneName() == "naboo") {
		int randomShrine = System::random(3);

		if (randomShrine == 0)
			coordinate->setPosition(-6859, 0, -1937);
		else if (randomShrine == 1)
			coordinate->setPosition(-2582, 0, -6184);
		else if (randomShrine == 2)
			coordinate->setPosition(2377, 0, -473);
		else
			coordinate->setPosition(7182, 0, -234);
	} else 	if (zone->getZoneName() == "talus") {
		int randomShrine = System::random(3);

		if (randomShrine == 0)
			coordinate->setPosition(-5785, 0, 4478);
		else if (randomShrine == 1)
			coordinate->setPosition(-5449, 0, -3239);
		else if (randomShrine == 2)
			coordinate->setPosition(318, 0, 5842);
		else
			coordinate->setPosition(5760, 0, -5208);
	} else 	if (zone->getZoneName() == "rori") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-6375, 0, 6403);
		else if (randomShrine == 1)
			coordinate->setPosition(-4496, 0, -7531);
		else if (randomShrine == 2)
			coordinate->setPosition(307, 0, -978);
		else if (randomShrine == 3)
			coordinate->setPosition(-926, 0, 6046);
		else
			coordinate->setPosition(6854, 0, -1221);
	} else 	if (zone->getZoneName() == "lok") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-2132, 0, 5938);
		else if (randomShrine == 1)
			coordinate->setPosition(5455, 0, 3805);
		else if (randomShrine == 2)
			coordinate->setPosition(-5806, 0, 1977);
		else if (randomShrine == 3)
			coordinate->setPosition(-3641, 0, -6030);
		else
			coordinate->setPosition(4978, 0, -5674);
	} else 	if (zone->getZoneName() == "yavin4") {
		int randomShrine = System::random(3);

		if (randomShrine == 0)
			coordinate->setPosition(-3362, 0, 6914);
		else if (randomShrine == 1)
			coordinate->setPosition(6455, 0, 6423);
		else if (randomShrine == 2)
			coordinate->setPosition(-4585, 0, -3761);
		else
			coordinate->setPosition(2389, 0, -4934);
	} else 	if (zone->getZoneName() == "endor") {
		int randomShrine = System::random(4);
		...
		if (randomShrine == 0)
			...
			coordinate->setPosition(5116, 0, 1923);
	} else 	if (zone->getZoneName() == "dathomir") {
		int randomShrine = System::random(4);

		if (randomShrine == 0)
			coordinate->setPosition(-4148, 0, 5926);
		else if (randomShrine == 1)
			coordinate->setPosition(1654, 0, -5765);
		else if (randomShrine == 2)
			coordinate->setPosition(3087, 0, 4887);
		else if (randomShrine == 3)
			coordinate->setPosition(5570, 0, -1514);
		else
			coordinate->setPosition(-4961, 0, -3493);
	}
	...
	player->switchZone(zone->getZoneName(), coordinate->getPositionX(), coordinate->getPositionZ(), coordinate->getPositionY(), 0);

	//Add Wounds to player
	player->addWounds(CreatureAttribute::HEALTH, 100, true, false);
	player->addWounds(CreatureAttribute::ACTION, 100, true, false);
	player->addWounds(CreatureAttribute::MIND, 100, true, false);
	player->addShockWounds(100, true);
	...
	//SWG Awakening Custom -- TEF System, Set Faction Player To Covert Upon Cloning
	if (ServerSettings::instance()->getTefEnabled()) {
		if (!ghost->getJediState() >= 4 && player->getFaction() != 0 && ghost->getFactionStatus() != FactionStatus::COVERT)
			ghost->setFactionStatus(FactionStatus::COVERT);
	} else {
		if (!ghost->getJediState() >= 4 && ghost->getFactionStatus() != FactionStatus::ONLEAVE)
			ghost->setFactionStatus(FactionStatus::ONLEAVE);
	}
