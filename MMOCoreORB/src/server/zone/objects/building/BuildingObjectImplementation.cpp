...
/* Default SWGEmu Building Ejection
...
*/
	...
	//SWG Awakening Custom -- Building Ejection
	Vector3 worldPosition = getWorldPosition();
	Vector3 ejectionPoint;
	...
	if (hasTemplateEjectionPoint()) {
		...
		Vector<PathNode*> exteriorPathNode = exteriorPathGraph->getGlobalNodes();

		FloorMesh* interiorFloorMesh = portalLayout->getFloorMesh(1);

		if (interiorFloorMesh == NULL)
			return worldPosition;

		PathGraph* interiorPathGraph = interiorFloorMesh->getPathGraph();

		for (int i = 0; i < exteriorPathNode.size(); ++i) {
			PathNode* exteriorNode = exteriorPathNode.get(i);
			PathNode* interiorNode = interiorPathGraph->getNode(exteriorNode->getGlobalGraphNodeID());

			if (interiorNode != NULL) {
				ejectionPoint = exteriorNode->getPosition();
...
bool BuildingObjectImplementation::isAllowedEntry(CreatureObject* player) {
	...
	if (getLotSize() > 0) {
		...
		//SWG Awakening Custom -- TEF System, Building Permissions
		if (ServerSettings::instance()->getTefEnabled()) {
			if ((player->getPvpStatusBitmask() & CreatureFlag::TEF) || (player->getPvpStatusBitmask() & CreatureFlag::BOUNTYTEF))
				return false;
		} else {
...
uint32 BuildingObjectImplementation::getMaximumNumberOfPlayerItems() {
	...
	//SWG Awakening Custom -- Increased Structure Storage, Defined In Server Settings Config
