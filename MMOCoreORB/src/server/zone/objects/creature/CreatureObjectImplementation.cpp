...
void CreatureObjectImplementation::feignDeath() {
	...
	//SWG Awakening Custom --  Temporary State Debuff Stack Fix
	buff->setSkillModifier("private_feign_death_damage_divisor", 4);
	buff->setSkillModifier("private_feign_death_damage_multiplier", 5);
	creo->addBuff(buff);
...
void CreatureObjectImplementation::setStunnedState(int durationSeconds) {
	...
	if (hasBuff(buffCRC)) {
		...
		//SWG Awakening Custom --  Temporary State Debuff Stack Fix
		multBuff->setSkillModifier("private_stunned_damage_divisor", 5);
		multBuff->setSkillModifier("private_stunned_damage_multiplier", 4);
...
void CreatureObjectImplementation::setIntimidatedState(int durationSeconds, int defenceDebuff, int damageDivisor) {
	...
	if(hasBuff(buffCRC)) {
		...
		state->setSkillModifier("private_melee_defense", defenceDebuff);
		state->setSkillModifier("private_ranged_defense", defenceDebuff);
		...
		//SWG Awakening Custom --  Temporary State Debuff Stack Fix
		multBuff->setSkillModifier("private_intimidate_damage_divisor", damageDivisor);
...
bool CreatureObjectImplementation::isAggressiveTo(CreatureObject* object) {
	...
	//SWG Awakening Custom -- Spawn Protection Aggressive Check
	...
	if (object->getFaction() != getFaction() && object->getFaction() != 0 && getFaction() != 0) {

		//SWG Awakening Custom -- TEF System, Creature Flag Aggressive Check
		if (ServerSettings::instance()->getTefEnabled()) {
			if (((pvpStatusBitmask & CreatureFlag::OVERT) || (pvpStatusBitmask & CreatureFlag::TEF) || (pvpStatusBitmask & CreatureFlag::CITYTEF)))
				return true;
		//Default SWGEmu Behavior
		} else {
	...
	//SWG Awakening Custom -- Bounty Target Is Always Aggressive To BH
...
bool CreatureObjectImplementation::isAttackableBy(TangibleObject* object, bool bypassDeadCheck) {
	...
	//SWG Awakening Custom -- Check If Player Has Spawn Protection
...
bool CreatureObjectImplementation::isAttackableBy(CreatureObject* object, bool bypassDeadCheck) {
	...
	if (object->isPlayerCreature()) {
		...
		//SWG Awakening Custom -- Check If Player Has Spawn Protection
		if (targetGhost != NULL && targetGhost->hasSpawnProtection())
			return false;
	}
	...
	if (object->isAiAgent()) {
		...
		if (getFaction() == 0 || getFaction() == object->getFaction())
			...
			//SWG Awakening Custom -- TEF System, Creature Flag Check
			if (ServerSettings::instance()->getTefEnabled()) {
				if (!(pvpStatusBitmask & CreatureFlag::OVERT) && !(pvpStatusBitmask & CreatureFlag::TEF) && !(pvpStatusBitmask & CreatureFlag::CITYTEF))
					return false;
			//Default SWGEmu Behavior
			} else {
				if (getPlayerObject()->getFactionStatus() == FactionStatus::ONLEAVE)
					return false;
			}
		}
		return true;
...
bool CreatureObjectImplementation::isHealableBy(CreatureObject* object) {
	...
	//SWG Awakening Custom -- TEF System, Creature Flag Check
	if (ServerSettings::instance()->getTefEnabled()) {
		if (object->getFaction() == 0 && ((pvpStatusBitmask & CreatureFlag::OVERT) || (pvpStatusBitmask & CreatureFlag::TEF) || (pvpStatusBitmask & CreatureFlag::CITYTEF)))
			return false;
		if (getFaction() != object->getFaction() && ((pvpStatusBitmask & CreatureFlag::OVERT) || (pvpStatusBitmask & CreatureFlag::TEF) || (pvpStatusBitmask & CreatureFlag::CITYTEF)))
			return false;
		//Default SWGEmu Behavior
	} else {
...
void CreatureObjectImplementation::setFaction(unsigned int crc) {
	...
	if (isPlayerCreature()) {
		...
		//SWG Awakening Custom -- Join Factional Chat
		ChatManager* chatManager = server->getZoneServer()->getChatManager();

		String imperial = "imperial";
		String rebel = "rebel";

		if (crc == imperial.hashCode()) {
			//Join Imperial Chat
			ghost->addChatRoom(chatManager->getImperialRoom()->getRoomID());
		} else if (crc == rebel.hashCode()) {
			//Join Rebel Chat
			ghost->addChatRoom(chatManager->getRebelRoom()->getRoomID());
		} else {
			//Leave Faction Channels
			ghost->removeChatRoom(chatManager->getImperialRoom()->getRoomID());
			ghost->removeChatRoom(chatManager->getRebelRoom()->getRoomID());
		}

		//SWG Awakening Custom -- KnightTrials, Reset Trials On Faction Change
		String side = "";
		if (ghost->getKnightTrialState() > 15) {
			ghost->setKnightTrialState(15);
