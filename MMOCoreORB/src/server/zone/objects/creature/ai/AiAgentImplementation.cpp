...
bool AiAgentImplementation::isAggressiveTo(CreatureObject* target) {
	...
	if (getFaction() != 0 && targetFaction != 0) {
		...
		//SWG Awakening Custom -- TEF System, AI Attackable Check
		if (ServerSettings::instance()->getTefEnabled()) {
			if (ghost == NULL && targetFaction != getFaction())
				return true;
			else if (ghost != NULL && targetFaction != getFaction() && ((target->getPvpStatusBitmask() & CreatureFlag::OVERT) || (target->getPvpStatusBitmask() & CreatureFlag::TEF) || (target->getPvpStatusBitmask() & CreatureFlag::CITYTEF)))
				return true;
