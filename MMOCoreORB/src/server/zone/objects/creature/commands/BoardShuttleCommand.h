...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		//Do not allow shuttle usage if player is in combat with any kind of PvP flag.
		PlayerObject* ghost = creature->getPlayerObject();

		if (ghost == NULL)
			return GENERALERROR;

		//SWG Awakening Custom -- TEF System, Board Shuttle PvP Flag Check
		if ((creature->getPvpStatusBitmask() & CreatureFlag::OVERT) || (creature->getPvpStatusBitmask() & CreatureFlag::TEF) || (creature->getPvpStatusBitmask() & CreatureFlag::CITYTEF) || (creature->getPvpStatusBitmask() & CreatureFlag::BOUNTYTEF)) {
