...
protected:
	...
	//SWG Awakening Custom -- Intimidate Debuff Variables
	int intimDefenceDebuff;
	int intimDamageDivisor;
...
public:
	...
	CombatQueueCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
		...
		//SWG Awakening Custom -- Intimidate Debuff Variables
		intimDefenceDebuff = 0;
		intimDamageDivisor = 0;
	}
	...
	int doCombatAction(CreatureObject* creature, const uint64& target, const UnicodeString& arguments = "", ManagedReference<WeaponObject*> weapon = NULL) const {
		...
		//SWG Awakening Custom -- TEF System, Update TEF On Combat Action
		String rebel = "rebel";
		String imperial = "imperial";
		bool shouldTef = false;

		if (ServerSettings::instance()->getTefEnabled()) {
			if (creature->isPlayerCreature() && (targetObject->isPet() || targetObject->isVehicleObject())) {
				...
				if (targetOwner != NULL && targetOwner->isPlayerCreature()) {
					if (!combatManager->areInDuel(creature, targetOwner)) {
						if (!creature->isInBountyMission(creature, targetOwner) && !creature->isInBountyMission(targetOwner, creature)) {
							shouldTef = true;
						}
					}
				}
			} else if (creature->isPet() && (targetObject->isPet() || targetObject->isVehicleObject())) {
				ManagedReference<CreatureObject*> creatureOwner = creature->getLinkedCreature().get();
				ManagedReference<CreatureObject*> targetOwner = targetObject.castTo<CreatureObject*>()->getLinkedCreature().get();

				if (creatureOwner != NULL && targetOwner != NULL && creatureOwner->isPlayerCreature() && targetOwner->isPlayerCreature()) {
					if (!combatManager->areInDuel(creatureOwner, targetOwner)) {
						if (!creatureOwner->isInBountyMission(creatureOwner, targetOwner) && !creatureOwner->isInBountyMission(targetOwner, creatureOwner)) {
							shouldTef = true;
						}
					}
				}
			} else if (creature->isPlayerCreature() && targetObject->isCreatureObject()) {
				ManagedReference<CreatureObject*> targetCreature = cast<CreatureObject*>(targetObject.get());

				if (targetCreature != NULL) {
					if (targetCreature->getFaction() == imperial.hashCode() || targetCreature->getFaction() == rebel.hashCode()) {
						if (!combatManager->areInDuel(creature, targetCreature)) {
							if (!creature->isInBountyMission(creature, targetCreature) && !creature->isInBountyMission(targetCreature, creature)) {
								shouldTef = true;
							}
						}
					}
				}
			} else if (creature->isPet() && targetObject->isCreatureObject()) {
				ManagedReference<CreatureObject*> creatureOwner = creature->getLinkedCreature().get();
				ManagedReference<CreatureObject*> targetCreature = cast<CreatureObject*>(targetObject.get());

				if (creatureOwner != NULL && targetCreature != NULL && creatureOwner->isPlayerCreature()) {
					if (targetCreature->getFaction() == imperial.hashCode() || targetCreature->getFaction() == rebel.hashCode()) {
						if (!combatManager->areInDuel(creatureOwner, targetCreature)) {
							if (!creatureOwner->isInBountyMission(creatureOwner, targetCreature) && !creature->isInBountyMission(targetCreature, creatureOwner)) {
								shouldTef = true;
							}
						}
					}
		...
		if (shouldTef && creature->isPlayerCreature()) {
			...
			if (owner != NULL && owner->isPlayerCreature()) {
				...
				if (ownerGhost != NULL) {
					...
					ownerGhost->updateLastPvpCombatActionTimestamp(5);
				}
			}
		}
	...
	virtual void applyEffect(CreatureObject* attacker, CreatureObject* defender, uint8 effectType, uint32 mod) const {
		...
		case CommandEffect::INTIMIDATE:
			defender->setIntimidatedState(duration, intimDefenceDebuff, intimDamageDivisor);
			break;
	...
	//SWG Awakening Custom -- Get Intimidate Debuff Variables
	void setIntimDefenceDebuff(int debuff) {
		intimDefenceDebuff = debuff;
	}
	void setIntimDamageDivisor(int debuff) {
		intimDamageDivisor = debuff;
	}
};
