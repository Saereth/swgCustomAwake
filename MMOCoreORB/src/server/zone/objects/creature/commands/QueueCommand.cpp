...
void QueueCommand::checkForTef(CreatureObject* creature, CreatureObject* target) const {
	...
	if (target->isPlayerCreature()) {
		...
		//SWG Awakening Custom -- TEF System, Check For TEF
		if (!CombatManager::instance()->areInDuel(creature, target) && targetGhost != NULL) {
			if ((target->getPvpStatusBitmask() & CreatureFlag::OVERT) || (target->getPvpStatusBitmask() & CreatureFlag::TEF) || (target->getPvpStatusBitmask() & CreatureFlag::CITYTEF))
		...
		if (owner != NULL && owner->isPlayerCreature()) {
			...
			if (!CombatManager::instance()->areInDuel(creature, owner) && ownerGhost != NULL) {
				if ((owner->getPvpStatusBitmask() & CreatureFlag::OVERT) || (owner->getPvpStatusBitmask() & CreatureFlag::TEF) || (owner->getPvpStatusBitmask() & CreatureFlag::CITYTEF))
					ghost->updateLastPvpCombatActionTimestamp(5);
			}
		}
	}
