...
public:
	...
	bool isValidGroupAbilityTarget(CreatureObject* leader, CreatureObject* target, bool allowPet) const {
		...
		//SWG Awakening Custom -- Limit Squad Leader Buff Range
		if (!checkDistance(leader, target, 192))
			return false;
		...
		//SWG Awakening Custom -- TEF System, Tef/Overt SL Check
		if (ServerSettings::instance()->getTefEnabled()) {
			if (leader->getFaction() == 0 && ((target->getPvpStatusBitmask() & CreatureFlag::OVERT) || (target->getPvpStatusBitmask() & CreatureFlag::TEF) || (target->getPvpStatusBitmask() & CreatureFlag::CITYTEF)))
				return false;
			if (leader->getFaction() != target->getFaction() && ((target->getPvpStatusBitmask() & CreatureFlag::OVERT) || (target->getPvpStatusBitmask() & CreatureFlag::TEF) || (target->getPvpStatusBitmask() & CreatureFlag::CITYTEF)))
				return false;
