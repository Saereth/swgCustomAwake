	...
	bool canPlayInstrument(CreatureObject* creature, const uint64& target) const {
		...
		if (instrument == NULL) {
			...
			if (nala != NULL && dynamic_cast<Instrument*> (nala.get())) {
				...
				//SWG Awakening Custom -- Players can not play instruments that do not belong to them unless on admin.
				ManagedReference<SceneObject*> instrumentParent = instrument->getParent();
				if (instrument->getSpawnerPlayer() == NULL) {
					if (instrumentParent != NULL && instrumentParent->isCellObject()) {
						CellObject* cell = cast<CellObject*>( instrumentParent.get());
						StructureObject* structureObject = dynamic_cast<StructureObject*>(cell->getParent().get().get());

						if (structureObject != NULL && !structureObject->isOnAdminList(creature)) {
							creature->sendSystemMessage("You must be on the admin list of this structure to use this instrument.");
							return false;
						}
					}
				}
