	...
	int doQueueCommand(CreatureObject* creature, const uint64& target,
		...
		try {
			...
			if (targetCreature->isPlayerCreature()) {
				...
				if (targetGhost == NULL || !targetGhost->getJediState() > 0 || !targetCreature->isInBountyMission(creature,targetCreature) || !creature->hasSkill("combat_bountyhunter_master")) {
				...
				if (ownerGhost == NULL || !ownerGhost->getJediState() > 0 || !creature->hasSkill("combat_bountyhunter_master")) {
					creature->sendSystemMessage("@trap/trap:sys_creatures_only");
					return GENERALERROR;
				}
			...
			int bHTrapIncrease = 0;

			if (creature->hasSkill("combat_bountyhunter_master")) {
				bHTrapIncrease = 25;
			}

			trappingSkill += bHTrapIncrease;
