...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& targetID, const UnicodeString& arguments) const {
		...
		// target must be a creature or jedi in BH mission
		...
		if (target != NULL && target->isPlayerCreature()) {
			...
			if (!targetGhost->getJediState() > 0 || !target->isInBountyMission(owner,target) || !owner->hasSkill("combat_bountyhunter_master")) {
				droid->showFlyText("npc_reaction/flytext","confused", 204, 0, 0);  // "?!!?!?!"
				return GENERALERROR;
			}
		}
