...
ConversationScreen* TrainerInfoScreenHandler::handleScreen(CreatureObject* conversingPlayer, CreatureObject* conversingNPC, int selectedOption, ConversationScreen* conversationScreen) {
	...
	//SWG Awakening Custom -- Jedi Skill Trainer Fix
...
ConversationScreen* TrainerTrainSkillScreenHandler::handleScreen(CreatureObject* conversingPlayer, CreatureObject* conversingNPC, int selectedOption, ConversationScreen* conversationScreen) {
	...
	//SWG Awakening Custom -- Forced Jedi Respec, Force Sensitive Skills Are Free For Jedi Who Are Forced To Respec
	String skillName = skill->getSkillName();
	if (ghost != NULL && ghost->jediRespecInProgress() && skillName.contains("force_sensitive_")) {
		if (SkillManager::instance()->getForceSensitiveSkillCount(conversingPlayer, false) < 24)
			isFree = true;
	}

	//Verify that player can train skill.
	if (SkillManager::instance()->canLearnSkill(skillName, conversingPlayer, isFree)) {
		canLearnSkill = true;
	} else {
		//Check if the player lacks skill points.
		playerLacksSkillPoints = SkillManager::instance()->fullfillsSkillPrerequisitesAndXp(skillName, conversingPlayer);
	}
	...
	if (canLearnSkill && enoughCredits) {
		...
		//Award skill.
		SkillManager::instance()->awardSkill(skillName, conversingPlayer, true, false, isFree);

		//Send message to client
		StringIdChatParameter systemMessageCredits("@skill_teacher:prose_pay");
		systemMessageCredits.setDI(skillCost);
		systemMessageCredits.setTO("@skl_n:" + skillName);
