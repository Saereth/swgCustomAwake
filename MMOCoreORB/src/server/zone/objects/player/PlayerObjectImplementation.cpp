...
int PlayerObjectImplementation::addExperience(const String& xpType, int xp, bool notifyClient) {
	...
	//SWG Awakening Custom -- Forced Jedi Respec, Bypass Experience Cap
	bool bypassXpCap = false;

	if (jediRespecInProgress() && xpType == "jedi_general")
		bypassXpCap = true;
...
void PlayerObjectImplementation::notifyOnline() {
	...
	//SWG Awakening Custom -- Trigger The Awakening On Login Function
	awakeningOnLogin();
}
...
void PlayerObjectImplementation::addToBountyLockList(uint64 playerId) {
	...
	if (bountyLockList.contains(playerId)) {
		...
		//SWG Awakening Custom -- TEF System, Handle Bounty TEF
		if (ServerSettings::instance()->getTefEnabled()) {
			creature->setPvpStatusBit(CreatureFlag::BOUNTYTEF);
			target->setPvpStatusBit(CreatureFlag::BOUNTYTEF);
			updateInRangeBuildingPermissions();
...
void PlayerObjectImplementation::removeFromBountyLockListDirectly(uint64 playerId) {
	...
	//SWG Awakening Custom -- TEF System, Handle Bounty TEF
	if (ServerSettings::instance()->getTefEnabled()) {
		creature->clearPvpStatusBit(CreatureFlag::BOUNTYTEF);
		target->clearPvpStatusBit(CreatureFlag::BOUNTYTEF);
		updateInRangeBuildingPermissions();
		targetGhost->updateInRangeBuildingPermissions();
	}
...
void PlayerObjectImplementation::updateLastPvpCombatActionTimestamp(int duration) {
	...
	int tefDuration = (duration * 60000);

	lastPvpCombatActionTimestamp.updateToCurrentTime();
	lastPvpCombatActionTimestamp.addMiliTime(tefDuration);
...
//SWG Awakening Custom -- Awakening On Login Function
void PlayerObjectImplementation::awakeningOnLogin() {
	ManagedReference<CreatureObject*> creature = getParent().get().castTo<CreatureObject*>();
	...
	//SWG Awakening Custom -- Staff Permission Check
	ManagedReference<ZoneClientSession*> client = creature->getClient();
	int adminPermission = 0;

	if (ghost->hasAbility("admin")) {
		StringBuffer query;
		query << "SELECT admin_level FROM accounts WHERE account_id = '" << ghost->getAccountID() << "' LIMIT 1;";
		...
		if (adminPermission == 0) {
			//Remove Admin
			std::cout << adminPermission << std::endl;
			SkillManager::instance()->removeAbility(ghost, "admin");
			playerManager->updatePermissionName(creature, adminPermission);

			StringBuffer query1;
			query1 << "INSERT INTO account_bans values (NULL, " << ghost->getAccountID() << ", 1, now(), " << (uint64)time(0) + (60 * 525600) << ", 'Hacking');";
	...
	//SWG Awakening Custom -- Player Ip Address, Update Players Ip On Login
	Reference<BaseClientProxy*> session = creature->getClient()->getSession();
	String ipAddress = session->getIPAddress();
	setIpAddress(ipAddress);

	//SWG Awakening Custom -- TEF System, On Leave Check Upon Login
	if (creature->getFaction() != 0 && ghost->getFactionStatus() == FactionStatus::ONLEAVE)
		ghost->setFactionStatus(FactionStatus::COVERT);

	//SWG Awakening Custom -- Jedi System, Overt Check Upon Login
	if (ghost->getJediState() >= 4 && ghost->getFactionStatus() != FactionStatus::OVERT)
		ghost->setFactionStatus(FactionStatus::OVERT);

	//SWG Awakening Custom -- Knight Trials, Start The  Trials If Requirements Are Met
	if (creature->hasSkill("force_title_jedi_rank_02") && !creature->hasSkill("force_title_jedi_rank_03") && SkillManager::instance()->knightPrereqsMet(creature, "") && ghost->getKnightTrialState() == TrialState::KNIGHT_TRIAL_NOT_STARTED) {
	...
	//SWG Awakening Custom -- Knight Trials, Register Trial Creature Killed Observer If Trial In Progress
	ManagedReference<KnightTrialsObserver*> observer = new KnightTrialsObserver();

	if (ghost->getKnightTrialInProgress()) {
	...
	//SWG Awakening Custom -- Forced Jedi Respec, Trigger Forced Jedi Respec Function
	if (!ghost->hasGodMode() && !ghost->jediRespecInProgress() && creature->hasSkill("force_title_jedi_rank_01") && SkillManager::instance()->getForceSensitiveSkillCount(creature, false) < 24)
		ghost->checkFSSkillReqs();

	//SWG Awakening Custom -- Forced Jedi Respec, Trigger Respec Expired Function
	if (!ghost->hasGodMode() && ghost->jediRespecInProgress() && !ghost->hasJediRespecTime())
		ghost->jediRespecExpired();
	...
	if (ServerSettings::instance()->getEventSystemEnabled()) {
		...
		if (ServerSettings::instance()->getAwardBadgeEnabled()) {
			if (!ghost->hasBadge(awardedBadge))
				playerManager->awardBadge(ghost, awardedBadge);
		}
		...
		if (ServerSettings::instance()->getAwardItemEnabled()) {
			...
			StringBuffer query;
			query << "SELECT reward_claimed FROM accounts WHERE account_id = '" << ghost->getAccountID() << "' LIMIT 1;";
			ResultSet* result = ServerDatabase::instance()->executeQuery(query);
			...
			if (hasClaimedReward == 0) {
				...
				StringBuffer query1;
				query1 << "UPDATE accounts SET reward_claimed = '1' WHERE account_id = '" << ghost->getAccountID() << "'";
				ServerDatabase::instance()->executeStatement(query1);
			}
		}
	}
...
//SWG Awakening Custom -- Get Spawn Protection Timestamp Function
Time PlayerObjectImplementation::getSpawnProtectionTimestamp() {
	return spawnProtectionTimestamp;
}

//SWG Awakening Custom -- Update Spawn Protection Timestamp Function
...
//SWG Awakening Custom -- Has Spawn Protection Function
bool PlayerObjectImplementation::hasSpawnProtection() {
	return !spawnProtectionTimestamp.isPast();
}

//SWG Awakening Custom -- Schedule Spawn Protection Removal Task Function
...
//SWG Awakening Custom -- Forced Jedi Respec Function
void PlayerObjectImplementation::checkFSSkillReqs() {
	ManagedReference<CreatureObject*> creature = getParent().get().castTo<CreatureObject*>();
	ManagedReference<PlayerObject*> ghost = creature->getPlayerObject();
	...
	int jediXp = 0;
	int curJediXp = ghost->getExperience("jedi_general");

	//Zero out negative Jedi experience
	if (curJediXp < 0)
		jediXp = abs(curJediXp);
	...
	//Iterate through all force_discipline skills
	for (int i = 0; i < copyOfList.size(); i++) {
		Skill* skill = copyOfList.get(i);
		String skillName = skill->getSkillName();
		if (skillName.contains("force_discipline_")) {
			//Remove all skills and refund all Jedi experience spent
			if (SkillManager::instance()->surrenderSkill(skill->getSkillName(), creature, true, true))
				jediXp += skill->getXpCost();
		}
	}
	...
	//Set Already Respeced
	ghost->setJediRespecInProgress(true);

	//Update Jedi Respec time limit
	ghost->updateJediRespecTime(172800000); //48 Hours

	//Set Player To Covert If Knight
	if (creature->hasSkill("force_title_jedi_rank_03")) {
		ghost->setFactionStatus(FactionStatus::COVERT);
	}

	//Refund Jedi Experience
	ghost->addExperience("jedi_general", jediXp);

	//Set Jedi State To 1
	ghost->setJediState(1);

	//Send SUI Message Box
	ManagedReference<SuiMessageBox*> box = new SuiMessageBox(creature, SuiWindowType::NONE);
	box->setPromptTitle("Jedi Message");
	StringBuffer msg;
	msg << "All Jedi are required to have 24 skill points spent on Force Sensitive skills. "
			<< "At this time, all of your Jedi skills have been removed and the Jedi experience used to train them has been refunded to you.\n\n"
			<< "You have been provided a limited time free FS Skill respec. What this means is, Noldan will train FS Skills without requiring FS experience. "
			<< "Once the 24 FS Skill requirement is met, you can visit your normal Jedi trainer to regain your Jedi skills.\n\n"
			<< "**IMPORTANT**: You have 48 hours to complete your Jedi Respec and meet Knight requirements if applicable. If you do not complete your respec within 48 hours, you will lose your free FS skill respec, "
			<< "Jedi experience and the Initiate/Padawan boxes. In addition, if you're a Knight and do not meet the Knight requirements, it will be removed.";

	box->setPromptText(msg.toString());
	ghost->addSuiBox(box);
 	creature->sendMessage(box->generateMessage());

	ManagedReference<WaypointObject*> waypoint = ghost->getZoneServer()->createObject(0xc456e788, 1).castTo<WaypointObject*>();
	Locker waypointLocker(waypoint);

	String planet = "dathomir";
	waypoint->setPlanetCRC(planet.hashCode());
	waypoint->setPosition(4900, 0, -3808);
	waypoint->setCustomObjectName("Force Sensitive Camp", false);

	ghost->addWaypoint(waypoint, true, true);

}

//SWG Awakening Custom -- Forced Jedi Respec Expired Function
void PlayerObjectImplementation::jediRespecExpired() {
	ManagedReference<CreatureObject*> creature = getParent().get().castTo<CreatureObject*>();
	ManagedReference<PlayerObject*> ghost = creature->getPlayerObject();
	...
	//Remove Knight
	if (creature->hasSkill("force_title_jedi_rank_03") && !SkillManager::instance()->knightPrereqsMet(creature, "")) {
		//Remove Knight Title Title
		creature->removeSkill("force_title_jedi_rank_03", true);
	}

	//Remove Jedi
	if (creature->hasSkill("force_title_jedi_rank_01") && SkillManager::instance()->getForceSensitiveSkillCount(creature, false) < 24) {
		SkillList* skillList = creature->getSkillList();
		Vector<String> listOfNames;
		skillList->getStringList(listOfNames);
		SkillList copyOfList;
		...
		//Iterate through all force_discipline skills and remove
		for (int i = 0; i < copyOfList.size(); i++) {
			Skill* skill = copyOfList.get(i);
			String skillName = skill->getSkillName();

			if (skillName.contains("force_discipline_")) {
				SkillManager::instance()->surrenderSkill(skillName, creature, true, true);
			}
		}

		//Remove Padawan Title If Applicable
		if (creature->hasSkill("force_title_jedi_rank_02"))
			creature->removeSkill("force_title_jedi_rank_02", true);

		//Remove Initiate Title
		creature->removeSkill("force_title_jedi_rank_01", true);

		//Remove Jedi Experience
		ghost->removeExperience("jedi_general", true);

		//Remove Player Knight Trial Variables
		if (ghost->getKnightTrialState() != TrialState::KNIGHT_TRIAL_NOT_STARTED) {
			String side = "";
	...
	//Set Jedi Respec In Progress To False
	ghost->setJediRespecInProgress(false);
}

//SWG Awakening Custom -- Get Knight Trials Shrine Coords Function
Vector3 PlayerObjectImplementation::getKnightTrialFSCoords() {
	return knightTrialFSCoords;
}

//SWG Awakening Custom -- Set Knight Trials Shrine Coords Function
void PlayerObjectImplementation::setKnightTrialFSCoords(const Vector3& shrine) {
	knightTrialFSCoords = shrine;
}
...
//SWG Awakening Custom -- Jedi System, Get Jedi Unlock Variables
