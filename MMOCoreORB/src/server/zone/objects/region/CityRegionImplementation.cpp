...
void CityRegionImplementation::notifyEnter(SceneObject* object) {
	...
	if (object->isCreatureObject()) {
		...
		//SWG Awakening Custom -- City Alignment System, Handle City TEF On Entry
		unsigned int cityAlignment = city->getFactionAlignment();
		String factionStringRebel = "rebel";
		String factionStringImperial = "imperial";
		String faction;

		if (cityAlignment == factionStringRebel.hashCode())
			faction = "Imperial Stronghold";
		else if (cityAlignment == factionStringImperial.hashCode())
			faction = "Rebel Stronghold";
		else
			faction = "Neutral";

		if (ServerSettings::instance()->getTefEnabled() && ServerSettings::instance()->getCityTefEnabled()) {
			if (creature->isPlayerCreature() && creature->getFaction() != cityAlignment && cityAlignment != 0
					&& creature->getFaction() != 0) {
				creature->setPvpStatusBit(CreatureFlag::CITYTEF);
				creature->sendSystemMessage("You have entered an " + faction + ". You have gained a PvP TEF while inside city limits.");
			}
		}
...
void CityRegionImplementation::notifyExit(SceneObject* object) {
	...
	if (object->isCreatureObject()) {
		...
		//SWG Awakening Custom -- City Alignment System, Handle City TEF On Departure
		if (creature->isPlayerCreature() && ghost != NULL) {
			if (creature->getPvpStatusBitmask() & CreatureFlag::CITYTEF) {
				creature->clearPvpStatusBit(CreatureFlag::CITYTEF);
...
String CityRegionImplementation::getRegionName() {
	...
	//SWG Awakening Custom -- City Alignment System, Append Faction Name Tag To City Name
	...
	if (cityAlignment == factionStringImperial.hashCode())
		faction = "\\#FFFF00[Imperial] ";
	else if (cityAlignment == factionStringRebel.hashCode())
