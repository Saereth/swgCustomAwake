...
void StructureMaintenanceTask::run() {
	//SWG Awakening Custom -- Structure Maintenance Log
	structureMaintenanceLog.setLoggingName("Structure Maintenance Log");
	StringBuffer structureMaintenanceFileName;
	structureMaintenanceFileName << "log/system/structure_maintenance.log";
	structureMaintenanceLog.setFileLogger(structureMaintenanceFileName.toString(), true);
	structureMaintenanceLog.setLogging(true);
	...
	/*
	//SWG Awakening Custom -- Inactive Player Structure Cleanup
	bool inactiveStucturesEnabled = ServerSettings::instance()->getEnableInactiveStructureCleanup();
	int inactiveDays = ServerSettings::instance()->getInactiveStructureCleanupTime();

	if (inactiveStucturesEnabled) {
		if (ghost->getDaysSinceLastLogout() >= inactiveDays) {
			structureMaintenanceLog.info(owner->getFirstName() + " has been inactive, destroying player structure with Object ID: " + String::valueOf(strongRef->getObjectID()));
			StructureManager::instance()->destroyStructure(strongRef);
			return;
		}
	}
